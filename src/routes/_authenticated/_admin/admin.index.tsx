...
export const Route = createFileRoute("/_authenticated/_admin/admin/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Draft, edit and publish kalvoteq insights articles from the editorial workspace." },
...
function AdminPage() {
  ...
  return (
    <>
      <PageHero eyebrow="Editorial" title="Your articles" intro="Everything you have written, drafted, or published to the kalvoteq insights archive.">
        <Button asChild>
          <Link to="/admin/new">
      ...
      <Section>
        ...
        {isLoading ? (
          ...
          <div className="mt-10 rounded-xl border border-dashed border-border p-16 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Start a new article and it will appear in this list.</p>
            ...
            {visible.map((post) => (
              <li key={post.id} className="flex flex-wrap items-center gap-4 p-5">
                <div className="min-w-0 flex-1">
                  <Link to="/admin/$id" params={{ id: post.id }} className="font-semibold hover:text-primary">
                    {post.title || "Untitled"}
                  </Link>
                  <p className="mt-1 truncate text-sm text-muted-foreground">{post.excerpt || "No excerpt yet"}</p>
